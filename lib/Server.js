...
function Server(config, environment, bookshelf) {
	...
	this.gameController.on('game.end', function(data) {
		...
		_this.soundPlayer.queue(slug(data.winnerName).toLowerCase() + '-won-the-game');
	});
	...
	this.gameController.on('game.gamePoint', function(data) {
		...
		_this.soundPlayer.queue('game-point-' + slug(data.playerName).toLowerCase());
	});
	...
	this.gameController.on('game.switchServer', function(data) {
		...
		_this.soundPlayer.queue(slug(data.playerName).toLowerCase() + '-to-serve');
	});
